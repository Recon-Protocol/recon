...
function formatSupply(value: number | undefined): string {
  ...
  if (kas >= 1_000_000_000) {
    return `${formatNumber(kas / 1_000_000_000)} Mrd. KAS`
  }

  if (kas >= 1_000_000) {
    return `${formatNumber(kas / 1_000_000)} Mio. KAS`
  }

  if (kas >= 1_000) {
    return `${formatNumber(kas / 1_000)} Tsd. KAS`
  }
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : 'Verbindung zur RECON API nicht möglich',
      )
    } finally {
      setLoading(false)
    }
  ...
  return (
    <main className="recon-app">
      ...
      <section className="hero-section">
        <div>
          <p className="eyebrow">
            RECON PROTOCOL / NETWORK INTELLIGENCE
          </p>

          <h1>
            KASPA <span>RECON</span>
          </h1>

          <p className="hero-description">
            Kontinuierliche Netzwerkintelligenz für die
            Kaspa-Proof-of-Work-Infrastruktur. Überwachung
            des Netzwerkzustands, der Versorgungsdynamik,
            der Hashrate und der Emissionsbedingungen.
          </p>
        </div>
        ...
        <div className="hero-status">
          ...
          <span className="hero-status-meta">
            SERVICE: {health?.service ?? 'recon-api'}
          </span>
        ...
        <article className="metric-card">
          ...
          <div className="metric-value">
            {hashrate?.hashrate !== undefined
              ? formatNumber(hashrate.hashrate)
              : '—'}
      ...
      <section className="dashboard-grid">
        <article className="panel">
          ...
          <div className="data-list">
            ...
            <div className="data-row">
              ...
              <strong>
                {metrics
                  ? formatNumber(metrics.header_count, 0)
                  : '—'}
              </strong>
            </div>
            ...
            <div className="data-row">
              ...
              <strong>
                {metrics
                  ? formatNumber(metrics.difficulty, 0)
                  : '—'}
              </strong>
            </div>
        ...
        <article className="panel wide-panel">
          <div className="panel-header">
            ...
            <span className="panel-status">
              {report?.network_status?.toUpperCase() ??
                '—'}
            </span>
          </div>
      ...
      <footer className="footer">
        <span>
          RECON-PROTOKOLL / KASPA-NETZWERK-INTELLIGENZ
        </span>
        ...
        <span>
          {loading
            ? 'SYNCHRONISIERE...'
            : lastUpdated
              ? `LETZTES UPDATE / ${lastUpdated.toLocaleTimeString(
                  'de-DE',
                )}`
              : 'WARTET AUF DATEN'}
        </span>
